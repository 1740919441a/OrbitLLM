...
class PowerSampler:
    ...
    def energy_between(self, start: float, end: float) -> tuple[float, float]:
...
def run_one(
    ...
    prefill_j = prefill_energy / max(context_len, 1) if prefill_energy > 0 else avg_power_prefill * (t1 - t0) / max(context_len, 1)
...
def run_model_quant(spec: dict[str, Any], quant: str, contexts: list[int], decode_tokens: int, runs: int) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    tokenizer, model = load_model(spec["model"], quant)
    for context_len in contexts:
        for seed in range(runs):
            rows.append(run_one(tokenizer, model, spec["model"], float(spec["scale_b"]), quant, int(context_len), decode_tokens, seed))
...
def main(argv: list[str] | None = None) -> int:
    ...
    for spec in model_specs:
        for quant in quants:
            rows.extend(run_model_quant(spec, quant, [int(c) for c in contexts], int(bench["decode_tokens"]), int(bench["runs"])))
