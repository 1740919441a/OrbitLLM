...
def plot_network_sensitivity(network: pd.DataFrame, fig_dir: Path) -> None:
    ...
    policy_order = ["All-Downlink", "All-On-Board", "Fixed-Threshold", "Priority-Aware", "Deadline-Energy", "Heuristic-TVD"]
    colors = {
        "All-Downlink": OK["red"],
        "All-On-Board": OK["orange"],
        "Fixed-Threshold": OK["green"],
...
def plot_tle_realism(tle: pd.DataFrame, contacts: pd.DataFrame, fig_dir: Path) -> None:
    ...
    labels = ["Down", "On", "Thresh", "Prio", "D-E", "TVD"]
...
def _heuristic_sensitivity_sentence(heuristic: pd.DataFrame | None) -> str:
    if heuristic is None or heuristic.empty or "normalized_value" not in heuristic.columns:
        return ""
    grouped = heuristic.groupby(["beta", "lambda_energy", "lambda_bandwidth"])["normalized_value"].mean()
    if grouped.empty:
        return ""
    best = float(grouped.max())
    worst = float(grouped.min())
    default_rows = heuristic[heuristic["is_default"] == 1] if "is_default" in heuristic.columns else pd.DataFrame()
    default = float(default_rows["normalized_value"].mean()) if not default_rows.empty else float(grouped.mean())
    gap = max(0.0, best - default)
    return (
        f"A 27-point TVD-weight sweep gives normalized value {worst:.2f}--{best:.2f}; "
        f"the default reaches {default:.2f} (gap {gap:.2f})."
    )


def write_tables(profile: pd.DataFrame, metrics: pd.DataFrame, out_dir: Path, heuristic: pd.DataFrame | None = None) -> None:
    ...
    for row in profile_table_rows(profile, "INT4", 2048):
        lines.append(
            ...
            f"{row['prefill_prompt_j']:.1f} & {row['decode_j']:.2f} & {row['peak_mem']:.1f}\\\\\n"
        )
    lines.append("}\n")
    ...
    for policy in order:
        ...
        if rows.empty or policy == "MILP-Ref":
            continue
    ...
    lines.append("}\n")
    lines.append(f"\\newcommand{{\\HeuristicSensitivitySummary}}{{{_heuristic_sensitivity_sentence(heuristic)}}}\n")
...
def main(argv: list[str] | None = None) -> int:
    ...
    heuristic_path = resolve_project_path(cfg.get("heuristic_sensitivity_path", "results/heuristic_sensitivity_metrics.csv"))
    ...
    heuristic = pd.read_csv(heuristic_path) if heuristic_path.exists() else None
    write_tables(profile, metrics, out_dir, heuristic)
